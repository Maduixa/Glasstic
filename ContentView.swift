...
class FastingManager: ObservableObject {
    ...
    private let sharedDefaults = UserDefaults(suiteName: "group.com.yourcompany.Glasstic")!
    ...
    func startFasting(goal: TimeInterval) {
        ...
        storedFastingState = fastingState.rawValue
        fastingGoal = goal
        elapsedTime = 0
        startTimer()
    ...
    private func startTimer() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1, repeats: true) { [weak self] _ in
            ...
            if self.elapsedTime >= self.fastingGoal {
                self.endFasting()
            }
        }
    }
    ...
    private func updateLiveActivity() {
        Task {
            ...
            let progress = elapsedTime / fastingGoal
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: UIApplication.willEnterForegroundNotification)) { _ in
            if fastingManager.fastingState == .fasting {
                // Timer will self-correct based on start date.
            }
        }
    }
...
struct FastingZoneInfoView: View {
    ...
    private var currentZone: FastingZone {
        // Find the highest zone that has been reached
        let reachedZones = FastingZone.allZones.filter { elapsedTime >= $0.duration }
        return reachedZones.last ?? .anabolic
    }
